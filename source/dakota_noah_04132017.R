...
# Load data----
# # Heart Failure (HF) scores
# scores.hf <- fread(file.path(DATA_HOME, "NJ HOSPITALS_Heart Failure Scores.csv"))
...
scores$year <- as.numeric(as.character(scores$year))
...
scores[, delta.overall := diff(range(overall, na.rm = TRUE)), by = hn]
scores
summary(scores)
...
ggplot(t3,
  ...
  scale_x_discrete(name = "MI Year",
                   breaks = unique(t3$Year),
                   labels = unique(t3$Year)) +
...
summary(dd)
write.csv(dd, "dakota.data.csv", row.names = F)
####################################################################
# ANOVA
